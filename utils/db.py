...
# ============================================================
# TEMPORARY MONGODB DIAGNOSTIC — remove after fixing
# ============================================================
st.markdown("## 🔧 MongoDB Diagnostic (temporary)")
try:
    _keys = list(st.secrets.keys())
    st.write(f"**1. Secrets keys found:** `{_keys}`")

    if "MONGO_URI" not in st.secrets:
        st.error("❌ MONGO_URI is MISSING from Streamlit Cloud secrets.")
    else:
        _uri = st.secrets["MONGO_URI"]
        # Show only the prefix — no password leak
        _prefix = _uri[:60]
        st.write(f"**2. MONGO_URI prefix:** `{_prefix}...`")

        try:
            from pymongo import MongoClient
            st.write("**3. Attempting ping to MongoDB...**")
            _c = MongoClient(_uri, serverSelectionTimeoutMS=8000)
            _result = _c.admin.command("ping")
            st.success(f"✅ MongoDB PING SUCCESSFUL — result: {_result}")
        except Exception as _e:
            st.error(f"❌ MongoDB PING FAILED — **{type(_e).__name__}**: {_e}")
except Exception as _outer:
    st.error(f"❌ Error reading secrets — {type(_outer).__name__}: {_outer}")
